...
{
	/*public partial class RoadBuilderUpdateNetSystem : GameSystemBase
	{
		...
		public Bounds3 GetTotalBounds(float expand = 0)
		{
			...
			{
				totalBounds = Encapsulate(totalBounds, GetMV(mvd).GetBounds());
			}

			if (expand != 0)
				totalBounds = Expand(totalBounds, expand);
		...
		protected override void OnUpdate()
		{
			...
			Dependency = UpdateCourse(Dependency, removeUpgrade: false);
		}
		...
		private JobHandle SnapControlPoints(JobHandle inputDeps, bool removeUpgrade)
		{
			...
			snapJob.m_Mode = actualMode;
			snapJob.m_Snap = GetActualSnap();
			snapJob.m_Elevation = elevation;
			snapJob.m_Prefab = m_PrefabSystem.GetEntity(m_Prefab);
			snapJob.m_LanePrefab = lanePrefab;
		...
		private JobHandle UpdateCourse(JobHandle inputDeps, bool removeUpgrade)
		{
			...
			if (m_Prefab != null)
			{
				...
				createDefinitionsJob.m_Mode = actualMode;
				createDefinitionsJob.m_ParallelCount = math.select(new int2(actualParallelCount, 0), new int2(0, actualParallelCount), m_CityConfigurationSystem.leftHandTraffic);
				createDefinitionsJob.m_ParallelOffset = parallelOffset;
				createDefinitionsJob.m_RandomSeed = m_RandomSeed;
		...
		private struct CreateDefinitionsJob : IJob
		{
			...
			private void CreateStraightLine(ref NativeParallelHashMap<Entity, OwnerDefinition> ownerDefinitions, int2 index)
			{
				...
				creationDefinition.m_SubPrefab = m_LanePrefab;
			...
			{
				float num = 0f;
				if ((placementFlags & (Game.Net.PlacementFlags.FlowLeft | Game.Net.PlacementFlags.FlowRight)) != 0)
				{
			...
			private void CreateSimpleCurve(ref NativeParallelHashMap<Entity, OwnerDefinition> ownerDefinitions, int middleIndex)
			{
				...
				creationDefinition.m_Flags |= CreationFlags.SubElevation;
				NetCourse course = default(NetCourse);
				course.m_Curve = NetUtils.FitCurve(new Line3.Segment(controlPoint.m_Position, controlPoint2.m_Position), new Line3.Segment(controlPoint3.m_Position, controlPoint2.m_Position));
			...
			private void CreateGrid(ref NativeParallelHashMap<Entity, OwnerDefinition> ownerDefinitions)
			{
				...
				bool ownerDefinition2 = GetOwnerDefinition(ref ownerDefinitions, Entity.Null, checkControlPoints: true, coursePos, coursePos2, out ownerDefinition);
				...
				while (int2.y <= @int.y)
				{
					...
					if (int2.y != @int.y)
					{
						...
						while (int2.x <= @int.x)
						{
							...
							component2.m_Prefab = m_NetPrefab;
							component2.m_SubPrefab = m_LanePrefab;
							component2.m_RandomSeed = random.NextInt();
							...
							if (ownerDefinition.m_Prefab != Entity.Null)
							{
								...
								if (m_EditorMode && GetLocalCurve(netCourse2, ownerDefinition, out var localCurveCache2))
								{
			...
			private void CreateContinuousCurve(ref NativeParallelHashMap<Entity, OwnerDefinition> ownerDefinitions)
			{
				...
				CreationDefinition creationDefinition2 = default(CreationDefinition);
				creationDefinition2.m_Prefab = m_NetPrefab;
				creationDefinition2.m_SubPrefab = m_LanePrefab;
				creationDefinition2.m_RandomSeed = random.NextInt();
